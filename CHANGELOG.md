...
- **Shell prompt git status checks inflated git usage by 10,000x** - Powerlevel10k and other prompt themes call `git status --porcelain` multiple times per second to display branch info, resulting in 30K+ git "usage" events per day (58 per minute) even while idle. The shim now filters automated prompt queries by detecting common patterns (`status --porcelain`, `rev-parse --git-dir`, `symbolic-ref --short HEAD`, etc.) and the `GITSTATUS_DAEMON` environment variable. Real user git commands (`git commit`, `git log`, `git push`) are still tracked normally. Users with existing inflated data can clean it with: `sqlite3 ~/.brewprune/brewprune.db "DELETE FROM usage_events WHERE package = 'git'"` then rebuild tracking from scratch.
...
- **Quick Start PATH instruction buried in IMPORTANT notice** (Round 13) - The PATH prerequisite appeared in an IMPORTANT notice above Quick Start, but not in the steps themselves. New users skimming the Quick Start might miss it and hit errors. Moved PATH requirement into step 2: "Ensure ~/.brewprune/bin is in PATH (quickstart does this automatically)". Agent B.
- **Quickstart PATH message is Linux-specific on macOS** (Round 13) - Quickstart printed "Added ... to PATH in ~/.profile" and mentioned brew services unavailability on all platforms, even though macOS uses different paths and does support brew services. Added `platformName()` helper that returns "(macOS)" or "(Linux)" based on runtime.GOOS and updated messages to show platform context. Agent C.
- **Quickstart warning banner 10 lines → too long** (Round 13) - The post-quickstart "TRACKING IS NOT ACTIVE YET" warning was 10 lines of dense text that users would skip. Condensed to 5 lines with clearer structure and action first: "Run this command now: source ~/.profile". Agent C.
- **"never" in Last Used column ambiguous when no usage data** (Round 13) - When no usage events existed (daemon hasn't run or just started), the "Last Used" column showed "never" for all packages, which reads as "never used" rather than "no data collected yet". Now shows "—" (em dash) when `hasUsageData == false`. Updated `RenderConfidenceTable` signature to accept `hasUsageData bool` parameter. Agent D.
- **Reclaimable space summary duplicated between header and footer** (Round 13) - `unused` output showed "Reclaimable: 39 MB (safe) · 248 MB (medium) · 66 MB (risky)" in the footer even when the header already displayed the same information in tier summary brackets. Removed footer line entirely to eliminate duplication. Agent D.
- **"Sorted by: score (highest first)" shown for default sort** (Round 13) - The sort annotation appeared even when sorting by score (the default), adding noise. Now only displays "Sorted by: ..." when sort order is non-default (size or age). Agent D.
- **Confidence section footer too verbose** (Round 13) - The bottom of `unused` output showed a "Breakdown:" section with score explanation repeated on every run. The warning banner already explains scoring. Simplified footer to just show Confidence line and Tip, reducing visual noise. Agent D.
- **explain didn't show usage history timeline** (Round 13) - `explain` showed usage points (40/40) but no usage history like "Used 5 times in last 7 days". Added usage timeline display: "Usage history: X total runs, last used Y ago" when usage data exists. Updated `renderExplanation` signature to accept `*analyzer.UsageStats` parameter. Agent E.
- **remove "locked by dependents" warning unclear** (Round 13) - "31 packages skipped (locked by dependents)" was ambiguous - are these packages with dependents, or dependents themselves? Clarified to "31 packages skipped (have other packages depending on them) - remove their dependents first, or run with --verbose to see details". Agent F.
- **remove --risky needs stronger warning** (Round 13) - Risky tier removal can break system dependencies but had no special warning beyond the status label. Added prominent warning banner at top of output: "⚠ WARNING: Risky tier removal may break system dependencies" with separator lines. Agent F.
- **undo --list doesn't show package names** (Round 13) - Snapshot list showed "5 packages" but not which ones, forcing users to run `undo <id>` without `--yes` to preview. Added `--verbose` flag to `undo --list` that expands each snapshot row to show package names. Agent F.
- **remove exit code 0 when all packages locked** (Round 13) - When no packages could be removed (all locked by dependents or already removed), `remove` exited 0 with success message. Now returns error and exits 1 when `len(packagesToRemove) == 0`. Agent F.
- **doctor lacks overall health score** (Round 13) - Individual checks showed ✓/⚠/✗ but no overall status. Added "Overall Status: HEALTHY" / "DEGRADED" / "BROKEN" line at top based on critical issue count. Green for HEALTHY (0 critical), yellow for DEGRADED (0 critical but warnings), red for BROKEN (1+ critical). Agent F.
- **doctor aliases tip doesn't show if aliases exist** (Round 13) - Tip to create `~/.config/brewprune/aliases` appeared even when file already exists. Now checks file presence and shows "ℹ Aliases configured (N mappings)" instead of tip when file exists. Agent F.
- **stats and status lack context counts** (Round 13) - Commands like `stats --all` didn't show "showing X of Y packages" context. Added "Showing N packages" line to stats and enhanced status with "N formulae · X KB" format. Agent F.
